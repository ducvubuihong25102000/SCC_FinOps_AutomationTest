...
When(/^91783 User navigate to General Journal$/, async () => {
  await pageFOHomePage.navigateTo(GENERAL_JOURNAL);
});
Then(/^91783 Verify user in which Legal Entity$/, async () => {
...
Then(
  /^91783 Verify user able to create General Journal for Bank and Posted$/,
  async () => {
...
Then(
  /^91783 Verify that a tick displays on the Posted column and the Posted Date$/,
  async () => {
...
Then(/^91783 Verify the value of the Voucher of the Posted GL$/, async () => {
  await browser.refresh();
