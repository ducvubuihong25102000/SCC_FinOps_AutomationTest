...
Then(
  ...
  async () => {
    InvoiceID = await pagePendingSupplierInvoice.createNewSupplierInvoice(
      ...
      dataVATScenario.GL_VAT[firstIndex].TCS92680[firstIndex].VATGroup
    );
  }
);
...
Then(/^92680 User Post the invoice$/, async () => {
  browser.refresh();
  await pagePendingSupplierInvoice.FilterInvoiceNumber(InvoiceID);
