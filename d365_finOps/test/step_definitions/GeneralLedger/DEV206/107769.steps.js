...
const DATA_107769 = require('../../../data/DEV206/107769.json');
...
Given(/^107769 User is on Purchase Order page$/, async () => {
  await PAGE_FINHOME.navigateTo(PURCHASE_ORDER);
});
When(
  /^107769 User create new Purchase Order with Project Category has no FDs info$/,
  async () => {
    PurchOrderID = await PAGE_PO.CreateNewPurchaseOrder(
      DATA_SUPP.Supplier,
      PURCHASE_ORDER,
      DATA_PO.CreatePurchaseOrder.Site,
      DATA_107769.TestInfo.Expected_Item,
      DATA_107769.TestInfo.Quantity,
      DATA_107769.TestInfo.Debit,
      DATA_107769.TestInfo.InputFinDim.No,
      DATA_FD.Branch,
...
Then(
  /^107769 User can see proper Financial Dimensions should be drived correctly$/,
  async () => {
    await PAGE_FINHOME.navigateTo(PURCHASE_ORDER);
    ...
    await PAGE_PO.Verify_Correct_Project_Category_And_Finacial_Dimensions_Are_Populated_On_Purchase_Order(
      DATA_107769.TestInfo.Expected_ProjectCategory,
      DATA_107769.TestInfo.Expected_FDs.Branch,
      DATA_107769.TestInfo.Expected_FDs.BusinessComponent,
      DATA_107769.TestInfo.Expected_FDs.BusinessUnit,
      DATA_107769.TestInfo.Expected_FDs.BusinessGroup,
      DATA_107769.TestInfo.Expected_FDs.CostCentre,
      DATA_107769.TestInfo.Expected_FDs.Manufacturer,
      DATA_107769.TestCase.ID
    );
    await PAGE_FINHOME.BackToHomePage();
    // //Write to file and assign Passed Status
    DATA_107769.TestCase.Status = isPassed;
    writeToCSV(DATA_107769.TestCase, `Test_Exec_Result.csv`);
  }
);
