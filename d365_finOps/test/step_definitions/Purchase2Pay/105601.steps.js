...
When(/^105601 User navigate to Pending supplier invoice$/, async () => {
  await pageFOHomePage.navigateTo(PENDING_SUPPLIER_INVOICES);
});

Then(
  /^105601 User Create a new Pending supplier invoice with any VAT code$/,
  async () => {
    let Number = await pagePendingSupplierInvoice.createNewSupplierInvoice(
      dataVATScenario.GL_VAT[firstIndex].TCS105616[firstIndex].InvoiceAccount,
      dataVATScenario.GL_VAT[firstIndex].TCS105616[firstIndex]
        .InvoiceDescription,
      dataVATScenario.GL_VAT[firstIndex].TCS105616[firstIndex].ItemNumber,
      dataVATScenario.GL_VAT[firstIndex].TCS105616[firstIndex].UnitPrice,
      dataVATScenario.GL_VAT[firstIndex].TCS105616[firstIndex].ItemVATGroup
    );

    let dataArray = dataVATScenario.GL_VAT[firstIndex].TCS105616;
...
Then(
  /^105601 User Open VAT transaction dialog and change Total calculated VAT amount$/,
  async () => {
...
Then(/^105601 User Post the invoice$/, async () => {
...
Then(/^105601 User navigate to Supplier$/, async () => {
  await pageFOHomePage.navigateTo(ALL_SUPPLIERS);
});
Then(/^105601 Open desire Supplier$/, async () => {
  await pageFilterInvoiceNumber.FilterInvoiceNumber();
});
Then(
  /^105601 Verify the voucher should be stored correct supplier invoice transaction$/,
  async () => {
