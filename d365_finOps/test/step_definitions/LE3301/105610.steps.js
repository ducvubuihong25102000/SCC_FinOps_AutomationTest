...
When(/^105609 User Navigate to All Purchase Order$/, async () => {
  await pageFOHomePage.navigateTo(PURCHASE_ORDER);
});

Then(
  /^Then 105610 User Opens the approriate purchase order with the Approval status is Approved$/,
  async () => {
...
Then(
  /^Then 105610 User Click on Request Change to the purchase order$/,
  async () => {
...
Then(
  /^Then 105610 Verify that the purchase order will now be changed to Draft state$/,
  async () => {
...
Then(/^Then 105610 User Click on Submit the PO again$/, async () => {
  await pageFOHomePage.navigateTo(ALL_SUPPLIERS);
});
Then(
  /^Then 105610 Verify that the purchase order is In Review state$/,
  async () => {
    await pageFilterInvoiceNumber.FilterInvoiceNumber();
  }
);
Then(/^Then 105610 Verify that the the PO status is Approved$/, async () => {
