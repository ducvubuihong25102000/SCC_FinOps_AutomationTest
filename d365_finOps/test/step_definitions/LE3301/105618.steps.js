...
When(/^105618 User navigate to Pending supplier invoice$/, async () => {
  await pageFOHomePage.navigateTo(PENDING_SUPPLIER_INVOICES);
});

Then(
  /^105618 User Create a new Pending supplier invoice with any VAT code$/,
  async () => {
...
Then(
  /^105618 User Open VAT transaction dialog and change Total calculated VAT amount$/,
  async () => {
...
Then(/^105618 User Post the invoice$/, async () => {
...
Then(/^105618 User navigate to Supplier$/, async () => {
  await pageFOHomePage.navigateTo(ALL_SUPPLIERS);
});
Then(/^105618 Open desire Supplier$/, async () => {
  await pageFilterInvoiceNumber.FilterInvoiceNumber();
});
Then(
  /^105618 Verify the voucher should be stored correct supplier invoice transaction$/,
  async () => {
