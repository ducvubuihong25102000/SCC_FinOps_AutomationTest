...
When(/^105614 User navigate to All Purchase Order$/, async () => {
  await pageFOHomePage.navigateTo(PURCHASE_ORDER);
});

Then(/^105614 User open the existed PO with status as Received$/, async () => {
...
Then(
  /^105614 User can generate PO Invoice for the Purchase Order$/,
  async () => {
...
Then(/^105614 Verify the PO status is moved to Invoiced$/, async () => {
  await pageFOHomePage.navigateTo(PURCHASE_ORDER);
  await pagePurchaseOrder.OpenPORecordViaFilter(
    dataPre[firstIndex].PurchaseOrderNumber
  );
...
Then(/^105614 User navigate to All Fixed Assets$/, async () => {
  await pageFOHomePage.navigateTo(FIXED_ASSETS);
  await pageFA.OpenFARecordViaFilter(dataPre[firstIndex].Fixedassetnumber);
});

Then(
  /^105614 Verify fixed asset has been acquired and placed in service date is populated$/,
  async () => {
