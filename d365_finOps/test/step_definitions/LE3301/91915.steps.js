...
Given(/^91915 User is on Purchase Order page$/, async () => {
  await browser.refresh();
  await pageFOHomePage.navigateTo(PURCHASE_ORDER);
});
When(
  /^91915 User open prepared PO with status Confirmed and it should have Yes, direct AICC other$/,
  async () => {
    await pagePurchaseOrder.OpenPORecordViaFilter(
      dataFA.FixedAssets[firstIndex].FABookForm[firstIndex].TC91846[firstIndex]
        .PurchaseOrderNumber
    );
  }
);
Then(
  /^91915 Verify that user can create a product receipt for that PO and generate a new Fixed asset successfully$/,
  async () => {
    ...
    let myFANum =
      await pagePurchaseOrder.Verify_FA_Should_Be_Created_After_PO_Is_Received(
        dataFA.FixedAssets[firstIndex].FABookForm[firstIndex].TC91846[
          firstIndex
        ].PurchaseOrderNumber
      );
    ...
    let fs = require('fs');
    fs.writeFile(
      '../d365_finOps/test/../data/91846_Precondition.json',
      jsonData,
      err => {
        if (err) throw err;
    ...
    // let myFANum = "FA00000381";
    await browser.refresh();
    await pageFOHomePage.navigateTo(FIXED_ASSETS);
    await pageFixedAsset.OpenFARecordViaFilter(myFANum);
...
When(/^91915 Navigate back to Purchase Order page$/, async () => {
...
Then(
  /^91915 Verify that user can create a supplier invoice for that PO successfully$/,
  async () => {
...
Then(
  /^91915 Navigate back to Fixed Asset page and open newly created FA$/,
  async () => {
    await pageLogin.openLink(global.baseUrl);
    await pageFOHomePage.navigateTo(FIXED_ASSETS);
...
Then(
  /^91915 Verify that FA should have status Open and have Acquisition price$/,
  async () => {
